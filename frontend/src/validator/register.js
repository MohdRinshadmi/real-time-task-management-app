...
export const loginPasswordValidator = (item, t) => {
    ...
    if (item?.validation?.minLength) {
        rules.minLength = {
            ...
            message: t ? t(`Minimum length is ${item.validation.minLength}`) : `Minimum length is ${item.validation.minLength}`,
        };
    }
    if (item?.validation?.maxLength) {
        rules.maxLength = {
            value: item.validation.maxLength,
            message: t ? t(`Maximum length is ${item.validation.maxLength}`) : `Maximum length is ${item.validation.maxLength}`,
        };
    }
    let pattern = '^';
    if (item?.validation?.specialChar) pattern += '(?=.*[!@#$%^&*()_+\-=\[\]{};\':\"|,.<>\?])';
    ...
    if (pattern !== '^.*$') {
        try {
            rules.pattern = {
                ...
                message: t ? t('Password does not meet requirements') : 'Password does not meet requirements',
            };
        } catch (e) {
